...
namespace Menu {
int startMenu() {
    std::cout << "        NEURAL NETWORK ENGINE\n"
                "----------------------------------------- \n"
...
bool create_new_network_selected() {
    ...
    for (int i = 1; i < network_config.layers; i++) {
        ...
        layer_config.activation_function =
            Menu::createNewNetwork::enterActivationFunction(i);
    ...
    network_config.input_data_file_path =
        Menu::createNewNetwork::enter_input_data_file_path();
...
std::string enter_input_data_file_path() {
    ...
    std::cout << "  - File should only contain float values seperated by spaces "
                "or newline characters\n";
    std::cout << "  - File should contain as many float values as size as input "
                "vector\n";
    std::cout << "Enter file path to input data file or enter 'e' to see an "
                "example format file: ";
